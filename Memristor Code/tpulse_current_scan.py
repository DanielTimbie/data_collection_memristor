...
def meas_current_(sens, LNA, DMM, delay=.05, LNA_gui_var=None, DMM_gui_var=None, Sens_gui_var=None):
    ...
    while(Sens_Loop):   # we shoukld make sure that new bias voltage didnt make the implifier to saturete
        ...
        # at this point, mag is either > .3 or < -1  
        elif -1>np.log10(abs(Test1)) and sens > 9:
            sens -= 1
            sleep(delay+.1)

        # at this point, either mag > .3 or sens <= 0
    ...
    # the dark current is needed to get resistance